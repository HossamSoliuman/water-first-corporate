@extends('layouts.app')
@section('content')
    <section class="interior-hero flow-lines py-10 md:py-14"><div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><x-breadcrumbs :items="[['name' => 'Insights', 'url' => route('insights.index')], ['name' => $category->name]]" /><h1 class="mt-8 text-[clamp(1.75rem,3.6vw,3rem)] font-semibold text-ink-800">{{ $category->name }}</h1>@if ($category->description)<p class="mt-5 text-lg text-ink-500">{{ $category->description }}</p>@endif</div></section>
